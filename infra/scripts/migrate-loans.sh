...
SELECT
  ...
  END,
  CASE
    WHEN pp.STATUS <> 1 AND DATE(pp.PAYMENT_DATE) < CURDATE()
      THEN GREATEST(
        COALESCE(pp.NUMBER_OF_DAYS_OVERDUE, DATEDIFF(CURDATE(), DATE(pp.PAYMENT_DATE))),
        DATEDIFF(CURDATE(), DATE(pp.PAYMENT_DATE))
      )
    ELSE 0
  END,
