...
**Money/VND integer rule:** Mọi số tiền nghiệp vụ của VNFITE phải dùng VND số nguyên, không lưu hoặc hiển thị phần thập phân như `4.923.170,05 đ`. Khi tính lãi, phí phạt, lịch trả nợ, ví, biến động số dư, ledger, nạp/rút, giải ngân, đầu tư và hoàn trả nhà đầu tư, phải làm tròn `BigDecimal` về `scale=0` bằng `RoundingMode.HALF_UP` trước khi lưu DB, gọi TIKLUY/MB, phát event, hoặc trả API. Frontend/CMS/mobile format tiền VND không được hiển thị số lẻ; nếu nhận dữ liệu cũ có phần thập phân thì phải làm tròn khi hiển thị. Chỉ các giá trị tỷ lệ/phần trăm/lãi suất mới được giữ phần thập phân.
