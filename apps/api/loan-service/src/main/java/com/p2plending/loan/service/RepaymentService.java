...
public class RepaymentService {
    ...
    private RepaymentScheduleResponse toResponse(RepaymentSchedule s) {
        ...
        return RepaymentScheduleResponse.builder()
                ...
                .principalPaid(money(s.getPrincipalPaid()))
                .interestPaid(money(s.getInterestPaid()))
                .lateFee(money(s.getLateFee()))
                .lateFeeOutstanding(money(s.getLateFeeOutstanding()))
                .totalOutstanding(money(s.getTotalOutstanding()))
                .interestPenalty(money(s.getInterestPenalty()))
                .principalPenalty(money(s.getPrincipalPenalty()))
                .interestPenaltyPaid(money(s.getInterestPenaltyPaid()))
                .principalPenaltyPaid(money(s.getPrincipalPenaltyPaid()))
                .interestPenaltyOutstanding(money(s.getInterestPenaltyOutstanding()))
                .principalPenaltyOutstanding(money(s.getPrincipalPenaltyOutstanding()))
                .principalOutstanding(money(s.getPrincipalOutstanding()))
                .interestOutstanding(money(s.getInterestOutstanding()))
                .status(s.getStatus())
                .dpd(displayDpd)
                .paidAt(s.getPaidAt())
                .build();
